...
### v3.3.1 (2026-06-22)
- **버그 수정**: 지출 내역 수정 시 최신 평균환율로 원화 금액이 강제로 재계산되던 문제 해결 (기존 원화 금액 유지 및, 현지 금액 수정 시 과거 기록 당시의 환율 역산 적용)
- **버그 수정**: 원화 결제(KRW)로 기록된 항목을 현지 지출 항목으로 수정하지 못하던 문제 해결 (해당 지출 구간의 통화 종류를 기준으로 원화 결제 여부를 판단하도록 개선)
- **버그 수정**: 같은 날짜에 기록된 지출 내역들이 시간 순서대로 정렬되지 않고 뒤섞이던 현상 수정 (`createdAt` 기준 2차 정렬 추가)
